---
name: research
description: "Just the research phase of /team: recon (internal codebase map) + scout (external prior art, market context). No verdicts, no opinions."
...
- `scout` — external prior art and market scan (comparable products, pricing, positioning, transfer test)
...
- You want a clean codebase map and market scan without a verdict.
- You're going to write the pitch yourself and want raw inputs.
...
3. Output:
   ...
   - Scout's claim/mechanism/evidence (comparables, market context, transfer-test reasoning)
   - Gaps either flagged in their mapping
4. No synthesis, no committee.

## Handoffs

`/team:team` for the full council using these facts. `/team:business`, `/team:technical`, or `/team:skeptics` for a single-committee follow-up.
